...
class TestSplitNodesDelimiter(unittest.TestCase):
    ...
    def test_invalid_markdown(self):
        node = TextNode("This has `broken text", TextType.TEXT)
    ...
    def test_split_image(self):
        node = TextNode(
            "This text with an ![image](https://i.imgur.com/zjjcJKZ.png) and another ![second image](https://i.imgur.com/3elNhQu.png)",
            TextType.TEXT,
        )
        new_nodes = split_nodes_image([node])
        self.assertEqual(
            [
                TextNode("This text with an ", TextType.TEXT),
